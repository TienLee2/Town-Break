...
public class Shop3 : MonoBehaviour
{
    ...
    public void Buy()
    {
        ...
        if (Wallet.GetAmount() >= part.price)
        {
            ...
            PlayerPrefs.SetInt("PartBought-3" + partItems[partIndex].name, 1);
    ...
    public void Add()
    {
        if (PlayerPrefs.GetFloat("Counter3") == 0)
        {
            // Save added part value.
            PlayerPrefs.SetInt("PartAdded-3" + partItems[partIndex].name, 1);
            // Load remove button.
            LoadButton();
            // Load rocket with added part.
            LoadRocket();
            PlayerPrefs.SetFloat("Counter3", 1);
        }
    }
    ...
    public void Remove()
    {
        ...
        PlayerPrefs.SetInt("PartAdded-3" + partItems[partIndex].name, 0);
        // Load add button.
        LoadButton();
        // Load rocket with removed part.
        LoadRocket();
        PlayerPrefs.SetFloat("Counter3", 0);
    }
    ...
    private void LoadRocket()
    {
        ...
        for (int i = 0; i < rocketItems.Count; i++)
        {
            ...
            bool partAdded = PlayerPrefs.GetInt("PartAdded-3" + partItems[i].name, 0) == 1 ? true : false;
    ...
    private void LoadButton()
    {
        ...
        bool partBought = PlayerPrefs.GetInt("PartBought-3" + partItems[partIndex].name, 0) == 1 ? true : false;
        if (partBought)
        {
            // Get value if part is added to the rocket.
            bool partAdded = PlayerPrefs.GetInt("PartAdded-3" + partItems[partIndex].name, 0) == 1 ? true : false;
            if (partAdded)
            {
